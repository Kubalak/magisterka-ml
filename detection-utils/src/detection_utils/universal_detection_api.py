...
class UniversalObjectDetector:
    ...
    def __init__(self, modelpath, category_index=None):
        ...
        if not os.path.exists(modeldir):
            raise FileNotFoundError(
                f"Model {modelpath} not found in exported models.")
        modeltype = None
        ...
        if os.path.exists(yolomp) and os.path.isdir(yolomp):
            if not os.path.exists(os.path.join(modelpath, "weights", "best.pt")):
                raise FileNotFoundError(
                    "No best.pt file found in yolo model dir.")
            modeltype = 'yolo'
        if os.path.exists(tfmp) and os.path.isdir(tfmp):
            if modeltype is not None:
                raise RuntimeError(
                    "Model directory contains both TensorFlow and YOLO models.")
            modeltype = 'tf'
        if modeltype is None:
            raise FileNotFoundError(
                "Selected model does not contain weights or checkpoint directory.")

        if modeltype == 'tf':
            self.__category_index = label_map_util.create_category_index_from_labelmap(
                category_index, use_display_name=True)
    ...
    def raw_detection(self, file: str | bytes, **kwargs):
        ...
        if not isinstance(file, str) and not isinstance(file, bytes):
            raise TypeError(
                "Invalid filename argument type! Allowed only str and bytes.")
        raw_content = file
        if type(file) is str:
    ...
    def __detect_tf(self, filename, th):
        ...
        boxes = self.__ExtractBBoxes(
            bboxes, bclasses, bscores, th, im_width, im_height, self.__category_index)
