...
def IoU(bbox_1: list, bbox_2: list):
    ...
    p1 = Polygon([[bbox_1[0], bbox_1[1]], [bbox_1[2], bbox_1[1]],
                 [bbox_1[2], bbox_1[3]], [bbox_1[0], bbox_1[3]]])
    p2 = Polygon([[bbox_2[0], bbox_2[1]], [bbox_2[2], bbox_2[1]],
                 [bbox_2[2], bbox_2[3]], [bbox_2[0], bbox_2[3]]])
...
def matrix_from_df(df: pd.DataFrame, require_class_max=True):
    ...
    results = [*map(lambda x: _row_support(x[1], x[1]
                    ['original_class'], require_class_max), df.iterrows())]
    results = np.array(
        [*map(lambda x: [classes_dict[x[0]], classes_dict[x[1]]], results)], dtype=np.int32)
