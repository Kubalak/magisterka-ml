...
def run_evaluation(modelname, time):
    ...
    with open(f"logs/{modelname}_{time}-stderr.log", "ab") as logfile:
        logfile.write(f'python {SCRIPT_NAME} --pipeline_config_path="models/{modelname}/pipeline.config" --model_dir="models/{modelname}" --checkpoint_dir="models/{modelname}" --alsologtostderr\n'.encode('utf-8'))
...
if __name__ == "__main__":
    ...
    comobox = ttk.Combobox(values=[*filter(lambda z: os.path.exists(os.path.join("models", z, "checkpoint")), os.listdir("models"))], width=50)
    comobox.place(x=20,y=50)
    
    button = ttk.Button(text="Start", command=lambda: run_eval_gui(comobox.get()))
