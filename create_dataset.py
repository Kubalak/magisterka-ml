...
if __name__ == "__main__":
    ...
    parser.add_argument("--basedir", "-bdir", help="Base dir to search for files", type=str, default="data")
    parser.add_argument("--filenum", "-fn", help="Number of files per class", type=int, default=10)
    parser.add_argument("--ratio", "-rt", help="How much data to training dataset [0-1]", type=float, default=0.8)
    parser.add_argument("--output_dir", "-od", help="Output dir to write files to (in workspace dir)", type=str, default="data")
    ...
    train_records = tf.data.TFRecordDataset([*map(lambda z: os.path.join('workspace', namespace.basedir, z), train_ds)])
    eval_records = tf.data.TFRecordDataset([*map(lambda z: os.path.join('workspace', namespace.basedir, z), eval_ds)])

    if not os.path.exists(os.path.join("workspace", namespace.output_dir)):
        os.makedirs(os.path.join("workspace", namespace.output_dir), exist_ok=True)
