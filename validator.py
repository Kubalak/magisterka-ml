...
def evaluate_detection(image_path, detector: UniversalObjectDetector, threshold: float, actual_class: str):
    ...
    # detection(image_path, width, height, model_fn, threshold, category_index)
    detections = detector.detect_objects(image_path, threshold)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model_path", help="Path to model eg. efficientdet_d0_coco17_tpu-32", type=str)
    parser.add_argument("--labels_path", "-lp", help="Path to labels map",
                        type=str, default="workspace/data/label_map.pbtxt")
    parser.add_argument("--threshold", "-th",
                        help="Detection threshold", type=float, default=0.2)
    parser.add_argument("--genbars", help="Generate bars for detections",
    ...
    detector = UniversalObjectDetector(os.path.join(
        "workspace", "exported_models", namespace.model_path), namespace.labels_path)
    ...
    with alive_bar(int(validations["class"].count())) as bar:
        for _, item in validations.iterrows():
            results.append(evaluate_detection(
                item["image"], detector, namespace.threshold, item["class"]))
            bar()

    results = pd.DataFrame(results)
    if namespace.genbars:
        print("Generating bars plot.")
        generate_bars(os.path.join("workspace", "exported_models",
                      namespace.model_path), results, True)

    results.to_csv(os.path.join("workspace", "exported_models",
                   namespace.model_path, "validation_results.csv"), index=False)
