...
def gen_iou(ious, modelname):
    ...
    with alive_bar(size) as bar:
        for a in range(size//25 + 1):
            ...
            f.text(0.5,0.9, f'Obszary wspólne i IoU dla {modelname}', verticalalignment='center', horizontalalignment='center')
            for i in range(5):
                for j in range(5):
                    if index < size:
                        item = ious[index]
                        draw_intersection(os.path.join('bricks', item['classname'], item['image']), item['gtbox'], item['pbox'], ax[i, j], (item['ioubox'], item['iou']))
                        bar()
                    else:
                        ax[i,j].set_xticks([])
                        ax[i,j].set_yticks([])
            ...
            plt.savefig(os.path.join("workspace", "exported_models", modelname, f"intersect_{a}.png"), bbox_inches='tight', pad_inches=0)
            plt.close(f)
            if index >= size:
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument("modelname", help="Name of model dir eg. yolov8n_size_416_bsize_16")
    parser.add_argument("--annotations_basedir", "-ab", help="Base dir for annotations if stored in non-default location", default="bricks", type=str)
    parser.add_argument("--draw", help="Draw IoU and export to image", action=argparse.BooleanOptionalAction)
    namespace = parser.parse_args()
    
    df = pd.read_csv(os.path.join("workspace", "exported_models", namespace.modelname, "validation_results.csv"))
    ...
    df['boxes'] = extract_class_detections(df.boxes.to_list(), df.original_class.to_list())
    ...
    results = [*map(lambda z: get_iou(namespace.annotations_basedir, z), flat_boxes)]
    if namespace.draw:
        # Draw intersections if param set
        gen_iou(results, namespace.modelname)
                                    
    results = pd.DataFrame(results)
    results.to_csv(os.path.join("workspace", "exported_models", namespace.modelname, "iou_results.csv"), index=False)
