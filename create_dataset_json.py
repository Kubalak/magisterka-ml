...
if __name__ == "__main__":
    ...
    dataset["train"] = [*map(lambda z: '.'.join([*z.split(".")[:-1], 'record']), dataset["train"])]
    
    dataset["eval"] = [*map(lambda z: z.split("/")[-1], dataset["eval"])]
    dataset["eval"] = [*map(lambda z: '.'.join([*z.split(".")[:-1], 'record']), dataset["eval"])]
    
    train_records = tf.data.TFRecordDataset([*map(lambda z: os.path.join('workspace', namespace.basedir, z), dataset["train"])])
    eval_records = tf.data.TFRecordDataset([*map(lambda z: os.path.join('workspace', namespace.basedir, z), dataset["eval"])])
